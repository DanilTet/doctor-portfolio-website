        ...
        if (article.show_in_blog && article.status === 'published') {
          ...
          if (!already) {
            posts.unshift({
              ...
              external_url: ['gastroscopy', 'colonoscopy', 'uzd', 'surgery'].includes(article.slug) ? `/${article.slug}/` : `/articles/${article.slug}`,
              date: article.date,
              source: 'article',
              tags: article.tags || [],
            });
      ...
      if (art.status === 'scheduled' && new Date(art.date) <= now) {
        ...
        if (art.show_in_blog) {
          ...
          const blogPost = {
            ...
            external_url: ['gastroscopy', 'colonoscopy', 'uzd', 'surgery'].includes(art.slug) ? `/${art.slug}/` : `/articles/${art.slug}`,
            date: art.date,
            source: 'article',
            tags: art.tags || [],
          };
